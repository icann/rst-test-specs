...
described in the `epp-02` test case. The test cases in the EPP test suite
ascertain whether these extensions are properlty implemented in the server.
...
extensions, the EPP test suite includes a set of input parameters which allow
test subjects to provide the extension element(s) which must be included in EPP
commands.

These input parameters **MUST** contain a single `<extension>` element which
